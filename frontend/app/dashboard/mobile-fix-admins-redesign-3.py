            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <style>{`
                @media (max-width: 768px) {
                  ...
                  .admin-mobile-cards-table thead {
                    display: none;
                  }
                  .admin-mobile-cards-table tr {
                    display: flex;
                  ...
                  .admin-mobile-cards-table td:last-child {
                    border-bottom: none;
                  }
                  .admin-mobile-cards-table td::before {
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border-light)', padding: '32px 24px', borderRadius: 'var(--radius-xl)', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginTop: '24px' }}>
                  {[
                    ...
                    { labelAr: 'الرتب المعطلة', labelEn: 'Inactive Ranks', count: adminRanks.filter(r => !r.is_active).length, icon: <PowerOff size={24} />, color: '#64748b', bg: 'rgba(100,116,139,0.1)' },
                  ].map((stat, idx) => (
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border-light)', padding: '24px', borderRadius: 'var(--radius-xl)' }}>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table className={`${styles.table} admin-mobile-cards-table`} style={{ margin: 0, width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ background: 'var(--bg-primary)' }}>
                      <tr>
                        <th style={{ padding: '16px', textAlign: locale === 'ar' ? 'right' : 'left' }}>{locale === 'ar' ? 'المستخدم' : 'User'}</th>
                        <th style={{ padding: '16px', textAlign: locale === 'ar' ? 'right' : 'left' }}>{locale === 'ar' ? 'الرتبة الحالية' : 'Current Rank'}</th>
                        <th style={{ padding: '16px', textAlign: locale === 'ar' ? 'right' : 'left' }}>{locale === 'ar' ? 'نوع الحساب' : 'Account Type'}</th>
                        <th style={{ padding: '16px', width: '300px', textAlign: locale === 'ar' ? 'right' : 'left' }}>{locale === 'ar' ? 'تعيين رتبة' : 'Assign Rank'}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {adminUsers
                        ...
                        <tr key={admin.id} style={{ borderBottom: '1px solid var(--border-light)' }}>
                          <td data-label={locale === 'ar' ? 'المستخدم' : 'User'} style={{ padding: '16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                              ...
                              <div style={{ overflow: 'hidden' }}>
                                <div style={{ fontWeight: 600, fontSize: '14px', color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{admin.profile?.full_name || admin.email.split('@')[0]}</div>
                                <div style={{ fontSize: '12px', color: 'var(--text-secondary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{admin.email}</div>
                              </div>
                            </div>
                          </td>
                          <td data-label={locale === 'ar' ? 'الرتبة الحالية' : 'Current Rank'} style={{ padding: '16px' }}>
                          ...
                          <td data-label={locale === 'ar' ? 'نوع الحساب' : 'Account Type'} style={{ padding: '16px' }}>
                            {admin.is_superuser
                          ...
                          <td data-label={locale === 'ar' ? 'تعيين رتبة' : 'Assign Rank'} style={{ padding: '16px' }}>
              ...
              <div>
                ...
                {adminRanks.length === 0 ? (
                  ...
                  <div className="admin-roles-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
                    {adminRanks
                        ...
                        <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                          ...
                          <div style={{ marginBottom: '20px', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                            {rank.name_en?.toLowerCase().includes('admin') || rank.name?.includes('مدير') ? (
                              <>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[المدفوعات]' : '[Payments]'}</span>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[الفواتير]' : '[Invoices]'}</span>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[العملاء]' : '[Customers]'}</span>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[الإعدادات]' : '[Settings]'}</span>
                              </>
                            ) : rank.name_en?.toLowerCase().includes('support') || rank.name?.includes('دعم') ? (
                              <>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[العملاء]' : '[Customers]'}</span>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[التذاكر]' : '[Tickets]'}</span>
                              </>
                            ) : rank.name_en?.toLowerCase().includes('dev') || rank.name?.includes('مطور') ? (
                              <>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[واجهة API]' : '[API]'}</span>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[الويب هوك]' : '[Webhooks]'}</span>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[السجلات]' : '[Logs]'}</span>
                              </>
                            ) : (
                              <>
                                <span style={{ fontSize: '11px', padding: '4px 8px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '6px', color: 'var(--text-secondary)' }}>{locale === 'ar' ? '[الصلاحيات الأساسية]' : '[Basic Permissions]'}</span>
                              </>
                            )}
                          </div>
                          ...
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <button onClick={() => {
                              ...
                              setIsRankModalOpen(true);
                            }} title={locale === 'ar' ? 'تعديل' : 'Edit'} className="btn btn-primary" style={{ flex: 1, padding: '8px', fontSize: '13px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                            ...
                            }} className="btn btn-outline" style={{ padding: '8px', borderRadius: '8px', color: rank.is_active ? '#f59e0b' : '#10b981' }} title={locale === 'ar' ? 'تعطيل / تفعيل' : 'Toggle Status'}>
                              {rank.is_active ? <PowerOff size={16} /> : <CheckCircle2 size={16} />}
                            </button>
                            <button onClick={() => {
                              if (membersCount > 0) {
                                alert(locale === 'ar' ? 'لا يمكن حذف هذه الرتبة لأنها مرتبطة بأعضاء، يمكنك تعطيلها بدلًا من حذفها.' : 'Cannot delete this rank because it is assigned to members. Disable it instead.');
                              } else {
                                handleDeleteRank(rank.id, rank.name);
                              }
                            }} className="btn btn-outline" style={{ padding: '8px', borderRadius: '8px', color: '#ef4444', borderColor: 'rgba(239,68,68,0.2)' }} title={locale === 'ar' ? 'حذف' : 'Delete'}>
                              <Trash2 size={16} />
                            </button>
                          </div>
                        </div>
    ...
    if m_start_idx != -1:
      ...
      {isRankModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div className="admin-rank-modal" style={{ background: 'var(--surface)', width: '100%', maxWidth: '600px', borderRadius: '24px', boxShadow: '0 24px 48px rgba(0,0,0,0.4)', border: '1px solid var(--border-light)', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
            ...
            <div style={{ padding: '24px', overflowY: 'auto' }}>
              ...
              <div style={{ background: 'var(--bg-primary)', borderRadius: '16px', padding: '24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', border: '1px solid var(--border-light)', marginBottom: '24px' }}>
                ...
                <div style={{ transform: 'scale(1.1)' }}>
                  <AdminBadge rank={{ name: (rankFormName || (locale === 'ar' ? 'اسم الرتبة' : 'Rank Name')) + (rankFormNameEn ? ' - ' + rankFormNameEn : ''), name_en: rankFormNameEn || 'Rank Name', color: rankFormColor, icon: rankFormIcon, is_active: rankFormActive }} size="md" />
                </div>
              </div>
              ...
              <form onSubmit={handleSaveRank} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                  <div>
                    ...
                    <input type="text" required value={rankFormName} onChange={e => setRankFormName(e.target.value)} placeholder="مثال: مشرف" style={{ width: '100%', padding: '10px 14px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '8px', color: 'var(--text-primary)', boxSizing: 'border-box', fontSize: '14px', outline: 'none', transition: 'border-color 0.2s' }} />
                  </div>
                  <div>
                    ...
                    <input type="text" value={rankFormNameEn} onChange={e => setRankFormNameEn(e.target.value)} placeholder="e.g: Supervisor" style={{ width: '100%', padding: '10px 14px', background: 'var(--bg-primary)', border: '1px solid var(--border-light)', borderRadius: '8px', color: 'var(--text-primary)', boxSizing: 'border-box', fontSize: '14px', outline: 'none', transition: 'border-color 0.2s' }} />
                  </div>
                </div>
...
"""
    ...
    print("Successfully updated page.tsx with refinements!")
