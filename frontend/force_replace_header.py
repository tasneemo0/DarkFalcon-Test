...
start_idx = content.find('<motion.header')
end_idx = content.find('</motion.header>') + len('</motion.header>')

if start_idx != -1 and end_idx != -1:
    new_header = '''        <header className={styles.topBar}>
          ...
          <div className={styles.topBarTitle}>
            ...
            <p>{locale === 'ar' ? أهلاً بك،  : Welcome back, }</p>
          </div>
    ...
    print("Could not find <motion.header block")
