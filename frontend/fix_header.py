...
if os.path.exists(PAGE_TSX):
    ...
    new_header = '''<motion.header
          className={${styles.topBar} flex items-center justify-between px-4 sm:px-6 w-full gap-4}
          style={{
        ...
        >
          {/* Right Section (in RTL) / Left Section (in LTR): Burger Menu + Title */}
          <div className="flex items-center gap-3 sm:gap-4">
            <button className={styles.burgerBtn} onClick={() => setSidebarOpen(true)}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="18" x2="21" y2="18" /></svg>
            </button>
            <div className={${styles.topBarTitle} flex flex-col items-start} style={{ textAlign: locale === 'ar' ? 'right' : 'left' }}>
              <h1 className="text-sm sm:text-base font-bold whitespace-nowrap m-0 text-[var(--text-primary)]">
                {isAdminMode ?
              ...
              <p className="text-xs sm:text-sm text-gray-500 whitespace-nowrap overflow-hidden text-ellipsis m-0 hidden sm:block">
                {locale === 'ar' ? 'إدارة لوحة التحكم الخاصة بك' : 'Manage your dashboard'}
              </p>
            </div>
          </div>

          {/* Left Section (in RTL) / Right Section (in LTR): Theme Toggle + Profile */}
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex flex-col items-end" style={{ textAlign: locale === 'ar' ? 'left' : 'right' }}>
    ...
    print("Fixed page.tsx header layout.")
